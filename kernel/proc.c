...
uint64 map_shared_pages(struct proc* src_proc, struct proc* dst_proc, uint64 src_va, uint64 size) {
  ...
  uint64 dst_end = dst_start + (src_end - src_start);
  uint64 pa;
  pte_t *src_pte;
  int flags;
  uint64 mapped_start = dst_start; 

  for (uint64 addr = src_start; addr < src_end; addr += PGSIZE) {
    src_pte = walk(src_proc->pagetable, addr, 0);
    if (src_pte == 0) {
      // Clean up already mapped pages
      if (dst_start > mapped_start) {
        uvmunmap(dst_proc->pagetable, mapped_start, (dst_start - mapped_start) / PGSIZE, 1);
      }
      return -1; 
    }
    ...
    if(mappages(dst_proc->pagetable, dst_start, PGSIZE, pa, flags|PTE_S) != 0){
      // Clean up already mapped pages
      if (dst_start > mapped_start) {
        uvmunmap(dst_proc->pagetable, mapped_start, (dst_start - mapped_start) / PGSIZE, 1);
      }
...
uint64 unmap_shared_pages(struct proc* p, uint64 addr, uint64 size) {
  ...
  uint64 original_sz = p->sz;  // Store original size
  ...
  for (uint64 a = start; a < end; a += PGSIZE) {
    ...
    if ((*pte & PTE_V) == 0 || (PTE_FLAGS(*pte) == PTE_V) || (*pte & PTE_S) == 0) {
      return -1; 
    }
  }
...
struct proc *find_proc_by_pid(int pid) {
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    acquire(&p->lock);
    if(p->pid == pid){
      // Found the process - return with lock held
      return p;
    }
    release(&p->lock);  // Release lock if not found
  }
  return 0;
}
